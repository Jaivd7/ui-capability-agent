...
export function invokePage(entry: CatalogEntryDetail, opts: InvokePageOptions): string {
  ...
  const fields = entry.inputParams.length
    ...
    : `<p class="text-sm text-slate-500">This capability takes no input parameters.</p>`;
  ...
  const form = `<form method="post" action="${action}" class="space-y-6">
    ...
    <div class="flex items-center gap-3 border-t border-slate-100 pt-5">
      ...
      <a href="/capabilities" class="text-sm text-slate-500 hover:text-slate-800">Cancel</a>
      <span class="ml-auto text-xs text-slate-400">POST ${escapeHtml(action)}</span>
    </div>
  </form>`;

  return `<div class="mb-6">
    <a href="/capabilities" class="text-xs text-slate-500 hover:text-slate-800">&larr; Capabilities</a>
    <div class="mt-2 flex flex-wrap items-center gap-2">
      <h1 class="text-xl font-semibold tracking-tight text-slate-900">${escapeHtml(entry.name)}</h1>
    ...
    <p class="mt-1 max-w-2xl text-sm text-slate-500">${escapeHtml(entry.description)}</p>
  </div>
  ${busyBanner}
...
const INPUT_CLASSES =
  "block w-full rounded-lg border-0 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-slate-900";
const INPUT_ERROR_CLASSES =
  "block w-full rounded-lg border-0 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm ring-1 ring-inset ring-red-400 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-red-500";
...
function field(param: InputParam, values: Record<string, string> | undefined, error: string | undefined): string {
  ...
  return `<div>
    <div class="flex flex-wrap items-center gap-2">
      <label for="${escapeHtml(id)}" class="text-sm font-medium text-slate-800">${escapeHtml(param.name)}</label>
    ...
    ${param.description ? `<small class="mt-0.5 block text-xs text-slate-500">${escapeHtml(param.description)}</small>` : ""}
...
function renderControl(
  ...
  if (param.type === "boolean") {
    ...
    return `<label class="inline-flex items-center gap-2 text-sm text-slate-700">
      ...
      <span class="text-slate-500">true when checked</span>
    </label>`;
  }
...
function roleField(entry: CatalogEntryDetail, opts: InvokePageOptions, error: string | undefined): string {
  ...
  return `<div>
    <div class="flex items-center gap-2">
      <label for="role" class="text-sm font-medium text-slate-800">Run as role</label>
      ${typeBadge("role")}
    </div>
...
function escalateField(values: Record<string, string> | undefined): string {
  ...
  return `<div>
    <div class="flex items-center gap-2">
      <span class="text-sm font-medium text-slate-800">Escalation</span>
      ${typeBadge("boolean")}
    </div>
    <small class="mt-0.5 block text-xs text-slate-500">Pause and hand the live browser session to you if the run gets stuck or hits an irreversible step.</small>
    <label class="mt-1.5 inline-flex items-center gap-2 text-sm text-slate-700">
...
function sidebar(entry: CatalogEntryDetail): string {
  const outputs = entry.outputs.length
    ? `<div class="flex flex-wrap gap-1.5">${entry.outputs
        ...
        .join(" ")}</div>`
    : `<p class="text-sm text-slate-400">This capability returns no outputs.</p>`;
  ...
  const outcomes = business.length
    ? `<ul class="space-y-2">${business
        .map(
          (o) => `<li>
            <span class="inline-flex items-center rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[11px] text-slate-700">${escapeHtml(
              o.code ?? o.id,
            )}</span>
            <p class="mt-0.5 text-xs text-slate-500">${escapeHtml(o.message ?? o.description)}</p>
          </li>`,
        )
        .join("")}</ul>`
    : `<p class="text-sm text-slate-400">No business outcomes recorded for this capability.</p>`;
  ...
  return `${card(
    ...
    "May return instead",
    `${outcomes}<p class="mt-3 border-t border-slate-100 pt-3 text-xs text-slate-500">A business outcome is a legitimate answer, not an error. It comes back with a code and a message and is reported as such.</p>`,
  )}${card(
    "Artifact",
    `<dl class="space-y-2 text-xs">
      <div class="flex justify-between gap-3"><dt class="text-slate-500">id</dt><dd class="font-mono text-slate-700">${escapeHtml(
        entry.id,
      )}</dd></div>
      <div class="flex justify-between gap-3"><dt class="text-slate-500">version</dt><dd class="font-mono text-slate-700">${escapeHtml(
        String(entry.version),
      )}</dd></div>
      <div class="flex justify-between gap-3"><dt class="text-slate-500">schema</dt><dd class="font-mono text-slate-700">${escapeHtml(
        entry.schemaVersion,
      )}</dd></div>
      <div class="flex justify-between gap-3"><dt class="text-slate-500">hash</dt><dd class="font-mono text-slate-700">${escapeHtml(
        shortHash(entry.contentHash),
      )}</dd></div>
      <div class="flex justify-between gap-3"><dt class="text-slate-500">target</dt><dd class="font-mono text-slate-700">${escapeHtml(
        entry.baseUrl,
      )}</dd></div>
      <div class="flex justify-between gap-3"><dt class="text-slate-500">steps</dt><dd class="font-mono text-slate-700">${escapeHtml(
