...
export function faultsPage(opts: {
  ...
  const body = `
    <p class="mb-4 text-sm text-slate-600">
      These controls drive the target's own <code class="rounded bg-slate-100 px-1">/settings</code> screen, using a
      separate short-lived session. They deliberately do <strong>not</strong> go through the browser context a
      capability runs in: <code class="rounded bg-slate-100 px-1">/settings</code> is left off the guardrail
    ...
    <form method="post" action="/faults" class="space-y-4">
      ...
      <div>
        <label class="block text-sm font-medium text-slate-800" for="forcedInject">Forced fault</label>
        <p class="mb-1 text-xs text-slate-500">Applied to every request until you set it back to none.</p>
        <select id="forcedInject" name="forcedInject" class="w-full max-w-lg rounded-md border border-slate-300 px-2 py-1.5 text-sm">${options}</select>
      </div>
      <div>
        <label class="block text-sm font-medium text-slate-800" for="errorRate">Random failure rate</label>
        <p class="mb-1 text-xs text-slate-500">0 to 1. Applies to posting actions only — useful for showing the recoverable tier retrying.</p>
  ...
  return `<div class="mb-6">
    <h1 class="text-xl font-semibold tracking-tight text-slate-900">Fault injection</h1>
